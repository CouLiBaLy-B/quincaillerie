...
def generate_receipt(sale_id):
    ...
    for item in sale.items:
        ...
        pdf.ln()
    pdf.ln(10)
    pdf.cell(200, 10, txt=f"Total de la vente: {sale.total_amount:.2f} EUR", ln=True, align='R')
...
def add_product():
    ...
    if not Product.query.filter_by(name=name).first():
        new_product = Product(name=name, category=category, stock_quantity=stock_quantity, min_price=min_price)
...
def import_excel_command():
    ...
    try:
        if not os.path.exists('sample_data.xlsx'):
            data = {
                'nom du produit': ['Marteau', 'Tournevis', 'Clous (boîte de 100)', 'Vis (boîte de 100)', 'Scie', 'Perceuse'],
                'catégorie': ['Outil à main', 'Outil à main', 'Quincaillerie', 'Quincaillerie', 'Outil à main', 'Outil électrique'],
