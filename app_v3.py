...
def dashboard():
    ...
    daily_revenue = db.session.query(func.sum(Sale.total_amount)).filter(func.date(Sale.sale_date) == today).scalar() or 0
    weekly_revenue = db.session.query(func.sum(Sale.total_amount)).filter(Sale.sale_date >= start_of_week).scalar() or 0
    monthly_revenue = db.session.query(func.sum(Sale.total_amount)).filter(Sale.sale_date >= start_of_month).scalar() or 0
    
    # Top products
    top_products = db.session.query(
        Product.name,
        func.sum(SaleItem.quantity_sold).label('total_sold')
    ).join(SaleItem).group_by(Product.name).order_by(func.sum(SaleItem.quantity_sold).desc()).limit(10).all()
    
    # Sales by category
    sales_by_category = db.session.query(
        Product.category,
        func.sum(SaleItem.quantity_sold * SaleItem.unit_price).label('category_revenue')
    ).join(SaleItem).group_by(Product.category).all()
    ...
    active_customers = Customer.query.filter(Customer.last_visit >= start_of_month).count()
...
def promotions():
    promotions_list = Promotion.query.filter(Promotion.active == True).order_by(Promotion.end_date).all()
    categories = db.session.query(Product.category).distinct().all()
    return render_template('promotions.html', promotions=promotions_list, categories=[c[0] for c in categories])
...
def sales_history():
    ...
    return render_template('sales_history.html', sales=sales, start_date=start_date, end_date=end_date)
...
def check_promotions():
    ...
    return jsonify({
        ...
        'promotion': applicable_promotion.to_dict() if applicable_promotion else None
    })
...
def process_sale():
    ...
    total_amount = sum(item['quantity'] * item['price'] * (1 - item.get('discount_percent', 0) / 100) for item in cart)
    discount_amount = sum(item['quantity'] * item['price'] * item.get('discount_percent', 0) / 100 for item in cart)
    ...
    try:
        ...
        db.session.commit()
        return jsonify({'message': 'Vente enregistrée avec succès !', 'sale_id': new_sale.id})
...
def add_customer():
    ...
    if request.is_json:
        return jsonify({'message': 'Client ajouté avec succès!', 'customer': customer.to_dict()})
    else:
...
def delete_supplier(supplier_id):
    ...
    if supplier.products:
        flash('Impossible de supprimer ce fournisseur car il a des produits associés.', 'danger')
    else:
...
def restock_product(product_id):
    ...
    if additional_stock > 0:
        ...
        db.session.commit()
        flash(f'Stock de {product.name} mis à jour! (+{additional_stock} unités)', 'success')
    else:
...
def sales_data():
    ...
    data = query.group_by(func.date(Sale.sale_date)).order_by(func.date(Sale.sale_date)).all()
...
def top_products_data():
    ...
    top_products = db.session.query(
        Product.name,
        func.sum(SaleItem.quantity_sold).label('total_sold'),
        func.sum(SaleItem.quantity_sold * SaleItem.unit_price).label('total_revenue')
    ).join(SaleItem).group_by(Product.name).order_by(func.sum(SaleItem.quantity_sold).desc()).limit(limit).all()
...
def generate_receipt(sale_id):
    ...
    # Sale info
    pdf.cell(200, 10, txt=f"Date: {sale.sale_date.strftime('%d/%m/%Y %H:%M:%S')}", ln=True)
    ...
    if sale.discount_amount > 0:
        pdf.cell(200, 10, txt=f"Remise: -{sale.discount_amount:.2f} EUR", ln=True, align='R')
    ...
    response.headers.set('Content-Disposition', 'inline', filename=f'recu_{sale_id}.pdf')
...
def export_inventory():
    ...
    for product in products:
        data.append({
            ...
            'Stock faible': 'Oui' if product.stock_quantity <= product.reorder_level else 'Non',
            'Date de création': product.created_at.strftime('%d/%m/%Y') if product.created_at else ''
        })
    ...
    response.headers['Content-Disposition'] = f'attachment; filename=inventaire_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
...
def export_sales():
    ...
    response = make_response(output.read())
    response.headers['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    response.headers['Content-Disposition'] = f'attachment; filename=ventes_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx'
...
def seed_data_command():
    ...
    # Add suppliers
    suppliers_data = [
        {'name': 'Outillage Pro', 'contact_person': 'Jean Dupont', 'email': 'contact@outillagepro.com', 'phone': '01.23.45.67.89'},
        {'name': 'Quincaillerie Centrale', 'contact_person': 'Marie Martin', 'email': 'marie@quinc-centrale.fr', 'phone': '01.98.76.54.32'},
        {'name': 'Électro Supply', 'contact_person': 'Pierre Durand', 'email': 'p.durand@electrosupply.com', 'phone': '01.11.22.33.44'}
    ]
    ...
    products_data = [
        {'name': 'Marteau professionnel', 'category': 'Outil à main', 'stock_quantity': 45, 'min_price': 12.99, 'supplier_id': supplier1.id, 'barcode': '1234567890123', 'reorder_level': 10},
        {'name': 'Tournevis cruciforme', 'category': 'Outil à main', 'stock_quantity': 75, 'min_price': 5.50, 'supplier_id': supplier1.id, 'barcode': '1234567890124', 'reorder_level': 15},
        {'name': 'Clous acier (boîte 100)', 'category': 'Quincaillerie', 'stock_quantity': 8, 'min_price': 3.20, 'supplier_id': supplier2.id, 'barcode': '1234567890125', 'reorder_level': 20},
        {'name': 'Vis inox (boîte 100)', 'category': 'Quincaillerie', 'stock_quantity': 150, 'min_price': 4.50, 'supplier_id': supplier2.id, 'barcode': '1234567890126', 'reorder_level': 25},
        {'name': 'Scie égoïne', 'category': 'Outil à main', 'stock_quantity': 25, 'min_price': 18.00, 'supplier_id': supplier1.id, 'barcode': '1234567890127', 'reorder_level': 8},
        {'name': 'Perceuse sans fil', 'category': 'Outil électrique', 'stock_quantity': 15, 'min_price': 89.99, 'supplier_id': supplier3.id, 'barcode': '1234567890128', 'reorder_level': 5},
        {'name': 'Pince universelle', 'category': 'Outil à main', 'stock_quantity': 30, 'min_price': 8.50, 'supplier_id': supplier1.id, 'barcode': '1234567890129', 'reorder_level': 12},
        {'name': 'Niveau à bulle', 'category': 'Outil de mesure', 'stock_quantity': 20, 'min_price': 15.75, 'supplier_id': supplier1.id, 'barcode': '1234567890130', 'reorder_level': 8}
    ]
    ...
    customers_data = [
        {'name': 'Jean-Claude Convenant', 'email': 'jc.convenant@email.com', 'phone': '06.12.34.56.78', 'address': '123 Rue de la Paix, Paris'},
        {'name': 'Sophie Bricolage', 'email': 's.bricolage@email.com', 'phone': '06.98.76.54.32', 'address': '45 Avenue du Travail, Lyon'},
        {'name': 'Paul Renovation', 'email': 'p.renovation@email.com', 'phone': '06.11.22.33.44', 'address': '78 Boulevard du Marteau, Marseille'},
        {'name': 'Marie Construction', 'email': 'm.construction@email.com', 'phone': '06.55.66.77.88', 'address': '12 Rue des Artisans, Toulouse'},
        {'name': 'Pierre Menuiserie', 'email': 'p.menuiserie@email.com', 'phone': '06.99.88.77.66', 'address': '34 Avenue des Outils, Nice'}
    ]
